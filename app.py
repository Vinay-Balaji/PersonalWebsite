...
BCOMCO (Bloomberg Brent Crude Subindex — Excess Return)
...
“BCOMCO”: pd.to_numeric(df.iloc[:, 1], errors=“coerce”),
“CO1”:    pd.to_numeric(df.iloc[:, 3], errors=“coerce”),
“CO12”:   pd.to_numeric(df.iloc[:, 5], errors=“coerce”),
})
result = result.dropna(subset=[“Date”]).sort_values(“Date”).reset_index(drop=True)
result[[“BCOMCO”,“CO1”,“CO12”]] = result[[“BCOMCO”,“CO1”,“CO12”]].ffill().bfill()
result = result.dropna(subset=[“BCOMCO”,“CO1”,“CO12”]).reset_index(drop=True)
return result
if n_cols >= 4:
test_dates = pd.to_datetime(df.iloc[:, 0], errors=“coerce”)
if test_dates.notna().sum() > len(df) * 0.5:
...
result = result.dropna(subset=[“Date”]).sort_values(“Date”).reset_index(drop=True)
result[[“BCOMCO”,“CO1”,“CO12”]] = result[[“BCOMCO”,“CO1”,“CO12”]].ffill().bfill()
result = result.dropna(subset=[“BCOMCO”]).reset_index(drop=True)
return result
...
if cl in [“BCOMCO”,“BCOMCO INDEX”]: col_map[col] = “BCOMCO”
elif cl in [“CO1”,“CO1 COMDTY”]: col_map[col] = “CO1”
elif cl in [“CO12”,“CO12 COMDTY”]: col_map[col] = “CO12”
df = df.rename(columns=col_map)
if “BCOMCO” not in df.columns:
st.error(“Missing BCOMCO column. Need BDH pulls: BCOMCO Index, CO1 Comdty, CO12 Comdty.”)
st.stop()
for col in [“BCOMCO”,“CO1”,“CO12”]:
if col in df.columns: df[col] = pd.to_numeric(df[col], errors=“coerce”)
if “CO1” not in df.columns: df[“CO1”] = df[“BCOMCO”]
if “CO12” not in df.columns: df[“CO12”] = df[“CO1”]
df[[“BCOMCO”,“CO1”,“CO12”]] = df[[“BCOMCO”,“CO1”,“CO12”]].ffill().bfill()
df = df.dropna(subset=[“BCOMCO”]).reset_index(drop=True)
return df
...
components = []
for lb in signals_config[“tsmom”][“lookbacks”]:
...
month_ends[“TMA”] = np.sign(prices - tma).loc[month_ends.index].values
...
df[“Market_Ret”] = df[“BCOMCO”].pct_change()
...
st.markdown(”**Brent Crude Oil CTA — BCOMCO Systematic Long/Short Strategy**”)
...
with st.sidebar:
    ...
    def rebalance(changed):
        new_val = min(max(st.session_state[f"w_{changed}"], 0), 100)
        ...
        actual = new_val + sum(st.session_state[f"w_{s}"] for s in others)
        if actual != 100:
            st.session_state[f"w_{others[0]}"] += 100 - actual
...
fig.add_trace(go.Scatter(x=df_bt["Date"], y=df_bt["BH_Cumulative"], name="Buy & Hold (BCOMCO)",
